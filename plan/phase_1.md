...
- [x] Write unit tests for auth endpoints:
  ...
  - [x] `POST /auth/login` — fails with 401 if username does not exist, response body contains `"Invalid credentials"`
  - [x] `POST /auth/login` — fails with 401 if password is wrong, response body contains `"Invalid credentials"`
...
- [x] Create `authStore.ts` — state: `token`, `user`; actions: `login`, `register`, `logout`, `restoreSession`
...
- [x] Create `LoginView.vue` — username + password form, calls `POST /auth/login`, navigates to HomeView on success; displays error message on 401
